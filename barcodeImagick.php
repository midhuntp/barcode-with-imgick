...
/**
 * php Imagick based Code 39 Barcode Image Generator
 *
 * @package Barcode39
 * @category Barcode39
 * @name Barcode39
 * @author Midhun TP  [thanks for shayanderson for his script to generate  Code 39 Barcode in GD ]
 * @version 1.0
 * @license http://www.gnu.org/licenses/gpl.html GPL License
 * This is free software and is distributed WITHOUT ANY WARRANTY
 *
 * width =260 px
 * height= 270 px [dyanamic size]
 * 300 dpi images
 * 32 bit images
 *
 *
 */
final class Barcode391 {
    ...
    public function draw($filename = null) {
                    ...
                    for ($j = 0; $j < strlen($code); $j++) {
                        ...
                        if (strlen($f2) == 2) {
                            ...
                            if ($w && $fill) {
                                ...
                                $bars[] = array($pos, $this->barcode_padding, $pos - 1 + $w, $this->barcode_height - $this->barcode_padding - 1, $fill);
        ...
        $strokeColor = new \ImagickPixel('#000000');
        $fillColor = new \ImagickPixel('#000000');
        ...
        $im = new Imagick();
        $im->newImage($bc_w, $this->barcode_height, new ImagickPixel('#000000'));
        $icc_cmyk = file_get_contents('./colorProfiles/USWebUncoated.icc');
        ...
        for ($i = 0; $i < count($bars); $i++) {
            ...
            if ($bars[$i][4] == "_000") {
                $fillColornew = new \ImagickPixel('#FFFFFF');
            } else {
                $fillColornew = new \ImagickPixel('#000000');
            }

            $draw->setFillColor($fillColornew);

            /* draw the line */
            $draw->line($bars[$i][0], $bars[$i][1], $bars[$i][0], $bars[$i][3]);
            $draw->line($bars[$i][0] + 1, $bars[$i][1], $bars[$i][0] + 1, $bars[$i][3]);

            $im->drawImage($draw);

            /* for adjusting the thickness */
            $lines = $bars[$i][2] - $bars[$i][0];
            if ($lines > 1) {
                $cnt = 1;
                ...
                while ($lines > 0) {
                    $lines--;
                    $draw->line($bars[$i][0] + $cnt, $bars[$i][1], $bars[$i][0] + $cnt, $bars[$i][3]);
                    $cnt++;
                    $im->drawImage($draw);
                }
            }
        ...
        /* returns Imagick object of generated barcode */
        return $im;
    }
